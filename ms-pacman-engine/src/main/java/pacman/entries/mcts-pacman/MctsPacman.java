...
public class MctsPacman extends Controller<MOVE> {
	...
	private static final int ghost_dist = 9;
	private static final int hunt_dist = 25;
	private static final int TREE_LIMIT = 35;
	...
	private MOVE FollowPath(MOVE dir, Game state) {
	...
	private MOVE MctsSearch(Game game) {
	...
	private MctsNode TreePolicy(MctsNode nd) {
		if(nd == null) {
			return null;
		}
	...
	private float DefaultPolicy(MctsNode nd) {
...
//		System.out.println(((float) state.getNumberOfActivePills() / ((float) pillsBefore)));
		return  1.0f -  ((float) state.getNumberOfActivePills() / ((float) pillsBefore));
	}

	private MctsNode BestChild(MctsNode nd, double C) {
...
//			System.out.println("Node: " + currentNode + "\t\tAction Move: " + currentNode.actionMove + "\t\tParent: " + currentNode.parent + "\t\tChildren Size: " + currentNode.children.toString() + "\t\tDelta Reward: " + currentNode.deltaReward);
